...
//#define EMIFStart 0x02000000
//#define DDRStart  0x80000000
...
{
  int status = E_PASS;
  volatile Uint32 bootcfg=0;

  bootcfg = SYSTEM->BOOTCFG;
...
Uint32 nand_writer()
{
  ...
  Int8  fileName[256],command[10];
  Int8  answer[24];
  Int32 i=0;
  Int8 ubl_path[60];
 // Uint32* ptr;
//  volatile Uint32 UBL_data[10] ={0};
  strcpy(ubl_path,"E:\\Projects\\CCS5.5\\system_test\\nand_ubl_write\\ubl.txt");
 ...
 // hNandInfo = NAND_open((Uint32)&EMIFStart, (Uint8) DEVICE_emifBusWidth() );
  ...
  DEBUG_printString("Writing UBL? (enter 'none' to skip) :\r\n");

  DEBUG_readString(command);

  if (strcmp(command,"none") != 0)
 {
  printf("Starting writing UBL ...\r\n");
//  strcpy(fileName,"E:\\Projects\\CCS5.5\\system_test\\nand_ubl_write\\ubl_DM36x_nand.bin");
//  strcpy(fileName,"E:\\Projects\\CCS5.5\\system_test\\nand_ubl_write\\ubl_432arm_340ddr_ipnc_dm368.bin");
  strcpy(fileName,"E:\\Projects\\CCS5.5\\system_test\\nand_ubl_write\\ubl_226_working.bin");
  fflush(stdin);
  if (strcmp(fileName,"none") != 0)
  {
    ...
    // UBL haeder used the page 0 ,need to be added
    ublAllocSize = (numPagesUBL+1) * hNandInfo->dataBytesPerPage;
 ...
 //   fp = fopen(ubl_path,"r+");
   // fseek(fp,0,SEEK_END);
 //   fwrite(ublPtr,ublAllocSize,1,fp);
 //   fclose(fp);
 //   ptr = (Uint32*)ublPtr;
 //   for(i = 0; i< 10;i++)
 //   {
...
//    }
 ...
 // DEBUG_readString(fileName);
  DEBUG_printString("Writing U-Boot? (enter 'none' to skip) :\r\n");

  DEBUG_readString(command);

  if (strcmp(command,"none") != 0)
 {
  printf("Starting writing U-Boot ...\r\n");
//  strcpy(fileName,"E:\\Projects\\CCS5.5\\system_test\\nand_ubl_write\\u-boot_dbg.bin");
 // strcpy(fileName,"E:\\Projects\\CCS5.5\\system_test\\nand_ubl_write\\u-boot-1.3.4-dm368_ipnc.bin");
 // strcpy(fileName,"E:\\Projects\\CCS5.5\\system_test\\nand_ubl_write\\u-boot.img");

  strcpy(fileName,"E:\\Projects\\CCS5.5\\system_test\\nand_ubl_write\\u-boot.bin");
...
NAND_WRITE_RETRY:
  ...
  do
  {
    DEBUG_printString((Int8 *)("Writing image data to Block "));
    DEBUG_printHexInt(blockNum);
    DEBUG_printString((Int8 *)", Page ");
    DEBUG_printHexInt(count & countMask);
    DEBUG_printString((Int8 *)"\r\n");
