...
static Uint32 *LOCAL_flashMakeAddr (Uint32 baseAddr, Uint32 offset);
...
//  dataword.l = data;
  ...
  addr = ( volatile Uint8* )( 0x02000000 + offset );
/**** modified by Harley*****/
	*addr = data;
...
//  cmdword.l = 0x0;
  volatile Uint8 data;
  volatile Uint8* addr;
  addr = ( volatile Uint8* )( 0x02000000 );

  //addr = LOCAL_flashMakeAddr (hNandInfo->flashBase, NAND_DATA_OFFSET );
/**** modified by Harley*****/
	data = *addr;
	return data;
...
static void LOCAL_flashWriteBytes(NAND_InfoHandle hNandInfo, void* pSrc, Uint32 numBytes)
{
  ...
  destAddr.lp = LOCAL_flashMakeAddr (hNandInfo->flashBase, NAND_DATA_OFFSET );
  switch (hNandInfo->busWidth)
  {
    case BUS_8BIT:
...
static void LOCAL_flashReadBytes(NAND_InfoHandle hNandInfo, void* pDest, Uint32 numBytes)
{
  ...
  srcAddr.lp = LOCAL_flashMakeAddr (hNandInfo->flashBase, NAND_DATA_OFFSET );
  switch (hNandInfo->busWidth)
  {
    case BUS_8BIT:
        for(i=0;i<( numBytes );i++)
...
static Uint32 LOCAL_flashGetDetails(NAND_InfoHandle hNandInfo)
{
  ...
  if(LOCAL_flashWaitForRdy(NAND_TIMEOUT) != E_PASS)
    return E_FAIL;
  /******************* Added by Harley **********************/
//	NANDFLASH_CLE = NAND_RESET;
//	dm368_waitusec(100000);

//   Start read operation
//    NANDFLASH_CLE = NAND_RDID;
//    NANDFLASH_ALE = 0x00;
...
//		mfgid = LOCAL_flashReadData(hNandInfo) & 0xFF;
//		devid = LOCAL_flashReadData(hNandInfo) & 0xFF;
//		nandinfo1 = LOCAL_flashReadData(hNandInfo) & 0xFF;
//		nandinfo2 = LOCAL_flashReadData(hNandInfo) & 0xFF;
//		nandinfo3 = LOCAL_flashReadData(hNandInfo) & 0xFF;
