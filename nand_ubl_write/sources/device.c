...
/* ARM 432MHz   DDR 340MHz    OSC 24MHz   */

#define PLL1_MUL		0x48 
#define PLL1_PREDIV		0x3
#define PLL1_POSTDIV	0x0 	// PVCO will 24*2M/N+1 = 864 Mhz

#define PLL1_DIV1		0x18	//  864/24  = 36 Mhz ->USB PHY 
#define PLL1_DIV2		0x1		//  864/2   = 432Mhz ->ARM926
#define PLL1_DIV3		0x2		//  864/3   = 288Mhz -> MJCP/HDVICP
#define PLL1_DIV4		0x5		//  864/6   = 144Mhz -> EDMA/Peripheral CFG0 (1/2 Kaleido clock)
#define PLL1_DIV5		0x2		//  864/3   = 288Mhz -> VPSS (max 270)
#define PLL1_DIV6		0x1F	//  864/32  = 27Mhz  -> VENC	
#define PLL1_DIV7		0x1		//  864/2/2 = 216Mhz -> DDRx2
#define PLL1_DIV8		0x8 	//  864/9   = 96 Mhz -> MMC0/SD0
#define PLL1_DIV9		0x3		//  864/4   = 216Mhz -> CLKOUT2 
#define PLL1_OSCDIV1    0x0		//  24/1    = 24Mhz	 -> CLKOUT0
...
#define PERIPHERAL_CLK_CTRL_VAL	0x043F0FF8
...
Uint32 DEVICE_init()
{
 ...
 // WDT_RESET();
  ...
  DEVICE_pinmuxControl(0,0xFFFFFFFF,0x00FD0000);  // Video Yin, SD0, McBSP, SD1_CLK
  DEVICE_pinmuxControl(1,0xFFFFFFFF,0x00145555);  // Video Cout, EXTCLK, FIELD
  DEVICE_pinmuxControl(2,0xFFFFFFFF,0x00001FFF);  // GPIO for LEDs,KEYs,EM_D[7:0]             
  DEVICE_pinmuxControl(3,0xFFFFFFFF,0x375AFFFF);  // SPI0, I2C, UART0, ENET, MDIO                            
  DEVICE_pinmuxControl(4,0xFFFFFFFF,0x00303CFF);  // CLKOUT0,(R1,R0,G1,G0,B1,B0-->LCD)

  GPIO_DIR23      = 0x1FFFFFFF;		//light the LED0,1,2;
  GPIO_CLR_DATA23 = 0xE0000000;
    ...
    PLL1->SECCTL = 0x00410000;   // Assert TENABLE = 0, TENABLEDIV = 0, TINITZ = 1
     ...
     // Post divider setting for PLL1 
		...
		/*Wait for PLL to LOCK */
          ...
          while(! (((SYSTEM->PLL0_CONFIG) & 0x0E000000) == 0x0E000000));
...
Uint32 DEVICE_PLL2Init()
{
	...
	UTIL_waitLoop(300); 
     ...
     UTIL_waitLoop(150);
      ...
      /*Wait for PLL to LOCK */
      while(! (((SYSTEM->PLL1_CONFIG) & 0x0E000000) == 0x0E000000)); 
