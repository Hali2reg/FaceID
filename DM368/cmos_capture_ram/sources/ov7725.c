...
Uint16 ov7725_init()
{
	...
	//check the camera ID , ov7725 should be 0x21.
	...
	// configure the registers of ov7725,including read-back checking.
...
//	Uint8 effect =0;
	...
	reg_nums = sizeof(reg_cfg_ycbcr)/sizeof(reg_cfg_ycbcr[0]);
	for(i = 0; i < reg_nums ; i++)
	{
		sccb_write_reg(reg_cfg_ycbcr[i][0],reg_cfg_ycbcr[i][1]);
		if(status != 0)
			return i;
		if(reg_cfg_ycbcr[i][1]!=sccb_read_reg(reg_cfg_ycbcr[i][0]))
			return i;
	}
