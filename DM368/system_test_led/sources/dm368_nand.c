...
Uint16 nand_read()
{
	...
	volatile Uint16 status =0;
	FILE *fp;
	Int8 s[100]  ="E:\\Projects\\CCS5.5\\system_test\\system_test_led\\nand.bin";
	//printf("select the file to write:\r\n");
	//scanf("%s",s);
	printf("nand read starting:\r\n");
	fp = fopen(s,"r+");
	if(fp == NULL)
		return 1;

	for(page = 2048 ;page < 3136; page++)
	{
		status = nand_readPage(page, dst, NANDFLASH_PAGESIZE);
		fseek(fp,0,SEEK_END);
		fwrite(dst,sizeof(dst),1,fp);
...
/*------------------------------------------------------------------------------
* Function   Name	: erase_all_blocks
* Despritions		: read operation to check NAND Flash UBL.
* Parameters		:
* Returned  Value	:
------------------------------------------------------------------------------*/
Uint16 erase_all_blocks()
{
	...
	for(i = 1 ; i < 8196; i++)
	{
		...
		if(status)
			printf("BAD BLOCKS NO: %d\r\n",i);

	}

	printf("Block NO 1~50 has been already erased!!!");
