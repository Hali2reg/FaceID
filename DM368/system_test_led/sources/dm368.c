...
void pll2_cfg()
{
	...
	PLL2_PLLDIV3    = 0x8000;   // 432/2  = 340Mhz   -> DDR (DDR has internal /2 divider)
	...
	//while(! (((PLL2_CONFIG) & 0x0E000000) == 0x0E000000));  // Wait for PLL to lock
	dm368_wait(2000);
...
//	pin_mux_cfg();
 ...
 //   PERI_CLKCTL = 0x04CF0FF8;          // DDR2 clock source   : PLLC1SYSCLK7
                                        // ARM926 clock source : PLLC1SYSCLK2
//	pll1_cfg();
//	pll2_cfg();
	
	dm368_gpio_init();		//LEDs
	...
	ae_cfg	= (Uint8)(BOOTCFG   &0x07);		//AEMIF config  0**-8bit width NAND
	
//	status = dm368_i2c_init();
//	status = dm368_i2c_write(0x55,data,1);
...
* Despritions		: leds shine in order.
* Parameters		: none
* Returned  Value	: none
------------------------------------------------------------------------------*/
void led_shining()
